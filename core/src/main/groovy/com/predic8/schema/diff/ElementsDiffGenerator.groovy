...
class ElementsDiffGenerator extends ListDiffGenerator {
  ...
  def removed = {
    new Difference(description: "${labelElement} ${it.name ?: 'ref to ' + it.refValue} with minOccurs ${it?.minOccurs} ${labelRemoved}.",
      type: 'element', breaks: ctx.exchange ? true : null, exchange: it.exchange)
  }

  def added = {
    new Difference(description: "${labelElement} ${it.name ?: 'ref to ' + it.refValue} with minOccurs ${it?.minOccurs} ${labelAdded}.",
      type: 'element', breaks: ctx.exchange ? true : null, exchange: it.exchange)
  }
