...
class SequenceDiffGenerator extends UnitDiffGenerator {
  ...
  def removed = {
    new Difference(description: "${labelSequenceRemoved}.", type: 'sequence', breaks: ctx.exchange ? true : null, exchange: a.exchange)
  }

  def added = {
    new Difference(description: "${labelSequenceAdded}.", type: 'sequence', breaks: ctx.exchange ? true : null, exchange: b.exchange)
  }
...
//    diffs.addAll(compareUnprocessedBPs(b.particles - bPs))
  ...
  private getParticleBIndex(aP) {
    if (aP instanceof Element) return b.particles.findIndexOf {
      it instanceof Element && (it.name ?: it.ref) == (aP.name ?: aP.ref)
    }
