...
class SchemaCreator extends AbstractSchemaCreator<SchemaCreatorContext> {
  ...
  void createElement(Element element, SchemaCreatorContext ctx) {
    ...
    if (element.minOccurs != '1') {
      attrs.put('minOccurs', element.minOccurs)
    }
    if (element.maxOccurs != '1') {
      attrs.put('maxOccurs', element.maxOccurs)
    }
  ...
  void createAttribute(Attribute attr, SchemaCreatorContext ctx) {
    ...
    if (attr.fixedValue) {
      attribs['fixed'] = attr.fixedValue
    }
    if (attr.use != 'optional') {
      attribs['use'] = attr.use
    }
    if (attr.defaultValue) {
      attribs['default'] = attr.defaultValue
    }
    if (attr.form) {
      attribs['form'] = attr.form
    }
    if (attr.arrayType) {
      attribs['wsdl:arrayType'] = attr.arrayType
    }
  ...
  void createAny(Any any, SchemaCreatorContext ctx) {
    def attrs = [:]
    if (any.namespace) attrs['namespace'] = any.namespace
    if (any.id) attrs['id'] = any.id
    if (any.processContents) attrs['processContents'] = any.processContents
    if (any.minOccurs != (String) 1) attrs['minOccurs'] = any.minOccurs
