...
class Definitions extends WSDLElement {
  ...
  def lookup = { item, qname ->
    registry.getWsdls(qname.namespaceURI)*."$item"?.flatten().find {
  ...
  Element getElement(GQName qname) {
    if (!qname) return
    def element
    (schemas + getSchemaLoadKnownSchemaIfNeeded(qname.namespaceURI) - null).findAll {
      it.targetNamespace == qname.namespaceURI
    }.each { schema ->
      try {
  ...
  Element getElementforOperationExchange(String ptName, String operationName, String exchange) {
    try {
      ...
      return bnd.operations.flatten().find { it.name == operationName }."$exchange".bindingElements.find {
        it instanceof SOAP11Body || it instanceof SOAP12Body
      }.parts[0].element
    } catch (Exception e) {
      return
    }
