...
class WSDLValidatorTest extends GroovyTestCase {
  ...
  void testValidation() {
    ...
    assert ctx.errors.grep(ValidationError).find {
      it.invalidElement instanceof Output
    }.cause instanceof NamespaceNotDeclaredForReferenceException
    ...
    assert ctx.errors.grep(ValidationError).find {
      it.invalidElement instanceof Input
    }.cause instanceof MessageAccessException
  ...
  void testBindingMissingOperation() {
    wsdl.validate(ctx)
    assert ctx.errors.grep(ValidationError).findAll {
      it.invalidElement.name == 'BindingMissingOperation'
    }[0].message ==
  ...
  void testPartValidation() {
    ...
    assert ctx.errors.grep(ValidationError).findAll {
      it.invalidElement instanceof Part
    }[0].cause instanceof ElementRefAccessException
    ...
    assert ctx.errors.grep(ValidationError).findAll {
      it.invalidElement instanceof Part
    }[1].cause instanceof TypeRefAccessException
