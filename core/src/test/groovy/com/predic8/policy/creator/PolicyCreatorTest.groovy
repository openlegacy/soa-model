...
class PolicyCreatorTest extends GroovyTestCase {
  ...
  void testPolicyCreatorFromWSDL() {
    ...
    assert wsdl.policies.values()*.allPolicyItems.size() == generatedWSDL.policies.values()*.allPolicyItems.size()
    // 3 items
    assert wsdl.policies.values()*.allPolicyItems.flatten().size() == generatedWSDL.policies.values()*.allPolicyItems.flatten().size()
    // 61 items
